...
class ServersListGUIDisplay<S>(
        ...
        for(i in 0 until (menuRows - 1) * 9) {
            if (i < page.items.size) {
                ...
                val item = if(customIcon == null) {
                    val itemSet = ItemStack(generalItem.material, min(max(if(generalItem.amount == -1) serverItem.getFilteredData(executor).provideData().size else generalItem.amount, 1), 64))
                    ...
                    val itemSet = ItemStack(customIcon.material, min(max(if(customIcon.amount == -1) serverItem.getFilteredData(executor).provideData().size else customIcon.amount, 1), 64))
    ...
    private fun serverTagResolvers(server: PlatformServerGroup<S>): Array<TagResolver> {
        return mutableListOf(
            ...
            TagResolver.resolver("player-count", Tag.inserting(Component.text(server.getFilteredData(executor).provideData().size))),
        ).toTypedArray()
    }
